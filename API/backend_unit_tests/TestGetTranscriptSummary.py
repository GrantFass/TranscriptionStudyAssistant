...
from responses import get_summary_response
import base64
...
class TestGetTranscriptSummary(unittest.TestCase):
    ...
    def test_valid_summary(self):
        ...
        response = get_summary_response(get_standard_body(self.ts, self.cId, self.username, self.token), valid_path)
        ...
        summary = response['p']['data']
        try: # Ensure text is base64 encoded
            self.assertTrue(base64.b64encode(base64.b64decode(summary.encode('ascii'))).decode('ascii') == summary)
        except Exception as e:
            self.fail(str(e))

        self.assertTrue('accuracy' in response['p'])
        self.assertTrue(isinstance(response['p']['accuracy'], float))
    ...
    def test_invalid_path(self):
        ...
        response = get_summary_response(get_standard_body(self.ts, self.cId, self.username, self.token), bad_path)
        self.assertTrue('err' in response)
        ...
        self.assertTrue('accuracy' not in response['p'])
