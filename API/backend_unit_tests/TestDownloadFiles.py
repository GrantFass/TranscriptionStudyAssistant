...
from responses import get_download_response
import base64
...
class TestDownloadFiles(unittest.TestCase):
    ...
    def test_download(self):
        ...
        response = get_download_response(get_standard_body(self.ts, self.cId, self.username, self.token),
                                              files=[{
                                                "path": "user/texts/khan1.txt",
                                              }])
        ...
        for file in response['p']['success']:
            ...
            self.assertTrue('filename' in file['data'])
            self.assertTrue(isinstance(file['data']['filename'], str))
            self.assertTrue('original_extension' in file['data'])
            self.assertTrue(isinstance(file['data']['original_extension'], str))
            self.assertTrue('processed' in file['data'])
            self.assertTrue(isinstance(file['data']['processed'], bool))
            self.assertTrue('text' in file['data'])
            self.assertTrue(isinstance(file['data']['text'], str))
