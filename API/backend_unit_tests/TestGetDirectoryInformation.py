...
from responses import get_directory_information_response
import base64


class TestGetDirectoryInformation(unittest.TestCase):
    def setUp(self):
    ...
    def test_directory_info(self):
        ...
        response = get_directory_information_response(get_standard_body(self.ts, self.cId, self.username, self.token))
        ...
        self.assertTrue('files' in response['p'])
        self.assertEqual(len(response['p']['files']), 1)
        for file in response['p']['files']:
