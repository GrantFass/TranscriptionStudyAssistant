...
class Preprocessor:
    ...
    def clean(self, text: str, tokenize=False, fast=False):
        text = self.clean_base(text)
        if not tokenize and not fast:
            doc = self.nlp(text)
            return doc
        elif not fast and tokenize:
            doc = self.nlp(text)
            clean_text = []
            for token in doc:
                flag = True
    ...
    def read_vtt_as_text(self, filepath: str):
        ...
        with open(filepath) as f:
            text = f.read()
        output_text = vtt_to_txt(text)
        return output_text
        
    def vtt_to_txt(self, text: str):
        """Method to read in a VTT transcript file and extract the text by removing timestamp / speaker tags"""
        lines = text.split('\n')
